class __javascript_strings {
    static __New() {
        ...
        String.Prototype.DefineProp('includes', {call: (this, value, start_pos:=unset) => InStr(this, value,, start_pos) ? 1 : 0})
        
        ; indexOf()
        ; Returns the index (position) of the first occurrence of a value in a string
        String.Prototype.DefineProp('indexOf', {call: (this, value, start_pos:=unset) => InStr(this, value,, start_pos)})
        
        ; lastIndexOf()
        ; Returns the index (position) of the last occurrence of a value in a string
        String.Prototype.DefineProp('lastIndexOf', {call: (this, value, start_pos:=unset) => InStr(this, value,, -1)})
        ...
        ; padEnd()
        ; Pads the end of the string to a given length with with a given string. Repeats if needed.
        String.Prototype.DefineProp('padEnd', {call: (this, length, pad:=' ') => pad(this, 'end', length, pad)})
        
        ; padStart()
        ; Pads the end of the string to a given length with with a given string. Repeats if needed.
        String.Prototype.DefineProp('padStart', {call: (this, length, pad:=' ') => pad(this, 'start', length, pad)})
        ...
        String.Prototype.DefineProp('slice', {call: (this, start, end:=unset) => SubStr(this, start, IsSet(end) ? end-start : unset)})
        ...
        pad(this, side, length, padding:=' ') {
            while (StrLen(this) != length)
                loop parse padding
                    if (side = 'Start')
                        this := A_LoopField this
                    else this .= A_LoopField
